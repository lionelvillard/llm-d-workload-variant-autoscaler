...
type InternalDecisionCache struct {
	sync.RWMutex
	items map[string]types.VariantDecision
}
...
func (c *InternalDecisionCache) Set(name, namespace string, d types.VariantDecision) {
	c.Lock()
	defer c.Unlock()
...
func (c *InternalDecisionCache) Get(name, namespace string) (types.VariantDecision, bool) {
	c.RLock()
	defer c.RUnlock()
	key := cacheKey(name, namespace)
	val, ok := c.items[key]
...
func DecisionToOptimizedAlloc(d types.VariantDecision) (*int32, string, metav1.Time) {
